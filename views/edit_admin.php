...
<title>ویرایش  مشخصات ادیمین ها</title>
	<style>
		*{
			margin: 0;
			padding: 0;
...
<body>
	<?php 
		...
		$_SESSION['csrf_edit_admin'] = $csrf_token;

    include("../partial/db.php");
    $id = $_GET['id'];
    $query = $conn->prepare("SELECT * FROM `users` WHERE `id` = ? AND `status` = ?");
    $query->execute([$id,"admin"]);
	...
	<div class="container">
		...
		<div class="form">
			...
			<form action="../controller/user_controller.php" method="post">
				...
				<input type="hidden" name="form-name" value="edit_admin">
                <input type="hidden" name="id" value="<?= htmlspecialchars($users[0][0]) ?>">
                <label for="username">نام ادمین: </label>
				...
				<br>
				<button type="submit">ویرایش  ادمین</button>
			</form>
		</div>
	</div>
</body>
